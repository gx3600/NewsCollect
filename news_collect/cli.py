...
@click.option(
    ...
    default=None,
    type=int,
    help="Max items per batch (default: from settings.yaml).",
)
@click.option(
    "--verbose", "-v",
    is_flag=True,
...
def analyze(batch_size: Optional[int], verbose: bool):
    """Run a single news analysis cycle: classify unprocessed news via LLM."""
    cfg = Config()
    setup_logging(level="DEBUG" if verbose else cfg.log_level)
    ...
    click.echo(f"Unprocessed news items: {unprocessed}")
    click.echo(f"Batch size: {analyzer.batch_size}, Concurrency: {analyzer.llm.concurrency}")

    if unprocessed == 0:
        click.echo("Nothing to process.")
        return

    click.echo("Running analysis cycle...")
    ...
    click.echo("=" * 50)
    click.echo(f"  Cycles run:       {stats['cycles']}")
    click.echo(f"  Total processed:   {stats['total_processed']}")
    click.echo(f"  Opinions stored:   {stats['total_opinions']}")
    click.echo(f"  Events stored:     {stats['total_events']}")
    click.echo(f"  Failures:          {stats['total_failures']}")
    click.echo()


@main.command(name="analyze-daemon")
@click.option(
    "--interval", "-i",
    default=None,
    type=int,
...
def analyze_daemon(interval: Optional[int], verbose: bool):
    """Run the news analysis task continuously on a schedule.

    Polls for unprocessed news at the configured interval and sends them
    to DeepSeek for classification. Press Ctrl+C to stop.
    """
    import signal
    import time
    ...
    analyzer = NewsAnalyzer()
    interval = interval or cfg.analysis_interval

    click.echo("Starting News Analysis daemon...")
    click.echo(f"  Interval:     {interval}s")
    click.echo(f"  Batch size:   {analyzer.batch_size}")
    click.echo(f"  Concurrency:  {analyzer.llm.concurrency}")
    click.echo(f"  Model:        {analyzer.llm.model}")
    click.echo("Press Ctrl+C to stop.")
    click.echo()
    ...
    click.echo("Running initial analysis cycle...")
    try:
        stats = analyzer.run_once()
        click.echo(
            f"  Processed: {stats['total_processed']}, "
            f"Opinions: {stats['total_opinions']}, "
            f"Events: {stats['total_events']}, "
            f"Failures: {stats['total_failures']}"
        )
    except Exception as e:
        logger.error(f"Analysis error: {e}", exc_info=verbose)

    # Schedule recurring
    schedule.every(interval).seconds.do(lambda: analyzer.run_once())

    while running:
        schedule.run_pending()
        time.sleep(1)

    click.echo("Analysis daemon stopped.")


@main.command(name="analysis-stats")
def analysis_stats():
    """Display statistics about analysis results."""
    cfg = Config()

    from news_collect.core.storage import NewsStorage

    storage = NewsStorage(cfg.db_path)
    stats = storage.analysis_stats()
    unprocessed = storage.unprocessed_count()

    click.echo("Analysis Statistics")
    click.echo("=" * 50)
    click.echo(f"  Unprocessed items:       {unprocessed}")
    click.echo(f"  Total opinion records:   {stats['total_opinions']}")
    click.echo(f"  Total event records:     {stats['total_events']}")
    click.echo(f"  Events affecting futures:{stats['events_affecting_futures']}")
    click.echo()

    if stats["opinions_by_variety"]:
        click.echo("Opinions by variety:")
        for variety, count in stats["opinions_by_variety"].items():
            click.echo(f"  {variety:20s}  {count:5d}")
