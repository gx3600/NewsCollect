"""上海有色网 (SMM) 五矿期货日报 spider — PDF+Image content via PyMuPDF."""
...
@register
class ShmetSpider(BaseNewsSpider):
    name: str = "shmet"
    source_name: str = "shmet"
    start_urls: list[str] = ["https://www.shmet.com/column/column-detail-1024-1008.html"]
    ...
    async def parse(self, response: Response) -> AsyncGenerator:
        logger.info(f"Crawling Shmet: {response.url}")
        body = response.body
        ...
        for m in re.finditer(r'<a\s+[^>]*href="(/news/newsDetail-\d+-\d+\.html)"[^>]*>(.*?)</a>', body, re.DOTALL):
            ...
            yield raw
            self.increment_new()
        logger.info(f"Shmet crawl complete: {self.stats['new']} new")
    ...
    async def _fetch_article_content(self, url: str, sid: str = "") -> str | None:
        try:
            ...
            # Fallback: collect COS image URLs
            img_urls = IMG_URL_RE.findall(body)
            if img_urls:
                unique = list(dict.fromkeys(img_urls))  # dedup preserving order
                return "[图片型文章 — 图片列表]\n" + "\n".join(unique[:20])
            return None
