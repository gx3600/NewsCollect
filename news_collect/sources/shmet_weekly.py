"""上海有色网 (SMM) 周报 spider — PDF+Image content via PyMuPDF."""
...
@register
class ShmetWeeklySpider(BaseNewsSpider):
    name: str = "shmet_weekly"
    source_name: str = "shmet_weekly"
    start_urls: list[str] = ["https://www.shmet.com/column/column-detail-1030-1008.html"]
    ...
    async def parse(self, response: Response) -> AsyncGenerator:
        logger.info(f"Crawling Shmet Weekly: {response.url}")
        body = response.body
        ...
        for m in re.finditer(r'<a\s+[^>]*href="(/news/newsDetail-\d+-\d+\.html)"[^>]*>(.*?)</a>', body, re.DOTALL):
            ...
            yield raw
            self.increment_new()
        logger.info(f"Shmet Weekly crawl complete: {self.stats['new']} new")
