"""上海有色网 (SMM) 品目连线日报 spider — PDF+Image content via PyMuPDF."""
...
@register
class ShmetReportSpider(BaseNewsSpider):
    name: str = "shmet_report"
    source_name: str = "shmet_report"
    start_urls: list[str] = ["https://www.shmet.com/column/column-detail-1027-1008.html"]
    ...
    async def parse(self, response: Response) -> AsyncGenerator:
        logger.info(f"Crawling Shmet Report: {response.url}")
        body = response.body
        ...
        for m in re.finditer(r'<a\s+[^>]*href="(/news/newsDetail-\d+-\d+\.html)"[^>]*>(.*?)</a>', body, re.DOTALL):
            ...
            yield raw
            self.increment_new()
        logger.info(f"Shmet Report crawl complete: {self.stats['new']} new")
